...
本脚本实现 WEEK_1_ANALYSIS_PLAN.md 中第 6 节和第 7 节：
1. 使用拟合前统计表中的 transmission 和 coin/hour 做指数衰减拟合；
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fit week-1 attenuation and analyze pulse-integral spectra."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
...
def fit_transmission(summary: pd.DataFrame) -> dict[str, float]:
    """使用 transmission 做带误差指数拟合。"""
    ...
    # lambda_eff 必须为正；初值取一个比最大厚度略大的尺度。
    popt, pcov = curve_fit(
        attenuation_model,
        x,
        y,
...
def fit_coin_rate(summary: pd.DataFrame) -> dict[str, float]:
    """使用 R_coin(counts/hour) 做带误差指数拟合。"""
...
def plot_fit(summary: pd.DataFrame, fit_result: dict[str, float], out_dir: Path) -> None:
    """绘制 transmission 拟合图和残差图。"""

    x = summary["lead_thickness_mm"].to_numpy(dtype=float)
    y = summary["transmission"].to_numpy(dtype=float)
    sigma = summary["sigma_transmission"].to_numpy(dtype=float)
    lambda_eff = fit_result["lambda_eff_mm"]
    ...
    ax.set_title("Week 1 attenuation fit")
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "week1_attenuation_fit.png", dpi=200)
    plt.close(fig)


def plot_coin_rate_fit(
    summary: pd.DataFrame, fit_result: dict[str, float], out_dir: Path
) -> None:
    ...
    y = summary["r_coin_per_hour"].to_numpy(dtype=float)
    sigma = summary["sigma_r_coin_per_hour"].to_numpy(dtype=float)
    r0 = fit_result["r0_counts_per_hour"]
    lambda_eff = fit_result["lambda_eff_mm"]
    ...
    ax.set_title("Week 1 coin/hour attenuation fit")
    ax.grid(True, alpha=0.3)
    ax.legend()
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "week1_coin_rate_fit.png", dpi=200)
    plt.close(fig)
...
def plot_coin_spectra(input_dir: Path, summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Week 1 coincidence-event area spectra")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "week1_area_coin_spectrum.png", dpi=200)
    plt.close(fig)
    ...
    ax.set_ylabel("Normalized counts")
    ax.set_title("Week 1 coincidence-event area2 spectra")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "week1_area2_coin_spectrum.png", dpi=200)
    plt.close(fig)
...
def main() -> None:
    ...
    summary_path = paths["tables"] / "week1_pre_fit_summary.csv"
    if not summary_path.exists():
        raise FileNotFoundError(f"Run week1_pre_fit_analysis.py first: {summary_path}")
    ...
    pd.DataFrame(fit_results).to_csv(paths["fit"] / "week1_attenuation_fit.csv", index=False)
    (paths["fit"] / "week1_attenuation_fit.json").write_text(
    ...
    spec_df.to_csv(paths["spectrum"] / "week1_spectrum_summary.csv", index=False)
    plot_coin_spectra(args.input_dir, summary, paths["spectrum"])

    print("week 1 fit and spectrum analysis completed")
    print(f"fit result: {paths['fit'] / 'week1_attenuation_fit.csv'}")
    print(f"spectrum summary: {paths['spectrum'] / 'week1_spectrum_summary.csv'}")

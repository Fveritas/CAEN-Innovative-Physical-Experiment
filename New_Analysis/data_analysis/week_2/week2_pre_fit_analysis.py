...
本脚本只完成指数衰减拟合之前的工作：
1. 读取第二周 51811、5185、5186、5187 的预处理 npz 数据；
2. 计算固定阈值下的基础计数、符合率、归一化符合比；
3. 以第二周 0 片铅板数据 51811 为基准，计算相对透过率和有效阻挡能力；
4. 生成基础统计表和诊断图。

注意：本脚本只分析第二周内部趋势，不进行跨周合并，也不进行指数衰减拟合。
"""
...
# 第二周包含 0、40、50、60 片铅板数据。51811 是第二周 0 片基准。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run pre-fit analysis for week-2 cosmic-ray lead data."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
...
def write_stats_table(stats: list[RunStats], table_dir: Path) -> pd.DataFrame:
    ...
    output_path = table_dir / "week2_pre_fit_summary.csv"
...
def plot_rate_curves(df: pd.DataFrame, figure_dir: Path) -> None:
    ...
    ax.set_title("Week 2 coincidence rate before attenuation fit")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(figure_dir / "week2_coin_rate.png", dpi=200)
    plt.close(fig)
    ...
    ax.set_title("Week 2 effective transmission and blocking")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(figure_dir / "week2_transmission_blocking.png", dpi=200)
    plt.close(fig)
    ...
    ax.set_title("Week 2 normalized coincidence ratios")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(figure_dir / "week2_normalized_coin_ratios.png", dpi=200)
    plt.close(fig)
    ...
    ax.set_title("Week 2 coincidence rate weighted by conditional ratio")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(figure_dir / "week2_conditional_weighted_coin_rate.png", dpi=200)
    plt.close(fig)
...
def plot_area_distributions(
    ...
    ax.set_title("Week 2 area distributions")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(figure_dir / "week2_area_distribution.png", dpi=200)
    plt.close(fig)
    ...
    ax.set_xlabel("area2")
    ax.set_ylabel("Normalized counts")
    ax.set_title("Week 2 area2 distributions")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(figure_dir / "week2_area2_distribution.png", dpi=200)
    plt.close(fig)
    ...
    for _, row in df.iterrows():
        ...
        fig.tight_layout()
        fig.savefig(figure_dir / f"week2_area_vs_area2_{run}.png", dpi=200)
        plt.close(fig)
...
def main() -> None:
    ...
    run_data = {run: load_run(args.input_dir, run) for run in WEEK2_RUNS}
    baseline_counts = compute_basic_counts(run_data[BASELINE_RUN])
    ...
    print("week 2 pre-fit analysis completed")
